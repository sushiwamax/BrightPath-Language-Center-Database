...
├── report.md / report.pdf       <- báo cáo cuối kỳ đầy đủ
├── noi_dung_bao_cao_cuoi_ky.md  <- nội dung văn xuôi từng chương (để dán vào Word)
├── erd.png                      <- sơ đồ EER final
├── 01_schema.sql
├── 02_seed_data.sql
├── 03_queries.sql
├── 04_views.sql
...
> ⚠️ **Lưu ý:** `01_schema.sql` bắt đầu bằng `DROP DATABASE IF EXISTS`,
