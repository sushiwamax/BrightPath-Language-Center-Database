...
-  ERD, mô hình quan hệ, chuẩn hóa 3NF (final, có change log)
-  10 bảng (`01_schema.sql`) — 9 bảng nghiệp vụ + 1 bảng audit mới
-  Dữ liệu mẫu đầy đủ (`02_seed_data.sql`)
-  8 business query: filter/order, join ≥3 bảng, left join, group+having, not exists, CTE, report theo thời gian, dùng view/function (`03_queries.sql`)
-  2 view phục vụ reporting (`04_views.sql`)
-  **3** stored procedure có transaction + validation (`sp_record_attendance`, `sp_cancel_meeting`, `sp_award_certificate`) + **3** stored function (`fn_student_attendance_count`, `fn_student_attendance_rate`, `fn_class_instructor`) — vượt yêu cầu tối thiểu 2 SP + 1 FN (`05_routines.sql`)
-  **4** trigger (kể cả 1 trigger chống "hồi sinh" buổi học đã kết thúc) + 1 event an toàn (disabled) (`06_triggers_events.sql`)
-  3 secondary index + `EXPLAIN`, kèm ghi chú về index tự sinh của FK (`07_indexes_explain.sql`)
-  Role/user least-privilege + backup/restore runbook (`08_admin_backup.md`)
-  **19** test case positive/negative (`09_tests.sql`)
